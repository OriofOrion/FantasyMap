...
ICON_DEFS.village.draw = function (ctx, s) { drawStamp(ctx, 'jans_village', s, INK); };
...
Object.assign(ICON_DEFS, {
  ...
  fort: { label: 'Fort', draw(ctx, s) { drawStamp(ctx, 'jans_fort', s, INK); } },
  wind_face: { label: 'Wind Face', defaultSize: 60, draw(ctx, s) { drawStamp(ctx, 'jans_wind_face', s, INK); } },
  palm_grove: { label: 'Palm Grove', defaultSize: 80, draw(ctx, s) { drawStamp(ctx, 'jans_palm_grove', s, '#4a6b3a'); } },
});

ICON_KEYS.push('ship', 'shipwreck', 'oasis', 'reef', 'wind_cherub', 'fort', 'wind_face', 'palm_grove');
